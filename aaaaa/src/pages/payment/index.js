...
const Payment = (props) => {
  ...
  const fetchArchieveToken = async (params = {}) => {
    ...
    let reservationObj = selectedPickUpPointsTr.length >= 1 && selectReturPickUs.length >= 1 ? reservations : [reservations[0]];
  ...
  useEffect(() => {
    ...
    dispatch({ type: "CHECK_JOURNEY_TYPE", payload: selectedPickUpPointsTr.length >= 1 && selectReturPickUs.length >= 1 ? 1 : 0 });
      ...
      <div className={`page ${styles.payment_page}`}>
        <div className={`page_section ${styles.payment_page_section}`}>
          ...
          >
            ...
            {selectReturPickUs.length > 0 &&
              ...
              selectReturnQuot && (
                <div className={styles.payment_details_content}>
                  ...
                  <div className={styles.reserv_details}>
                    <h3
                      style={{ marginTop: selectReturPickUs.length > 0 ? "3rem" : "0rem" }}
            ...
            <PaymentDetail env={env} setConfirmation={setConfirmation} />
          </div>
        </div>
      </div>
    </Layout>
