        ...
        if (Array.isArray(reservations[index][key])) {
          ...
          //!new addition
            ...
            if (checkTransferDetails === true) {
              ...
              if (typeof passengerDetails['getWhatsappNotifiations'] !== 'number' || ![1, 2].includes(passengerDetails['getWhatsappNotifiations'])) {
                errorHolder.status = 403;
                errorHolder.reservations[index][key]['getWhatsappNotifiations'] = appData?.words["strInvalidDataType"]
              }

              // check all keys
              let undifinedKeys = Object.keys(passengerDetails).filter(key => !['firstname', 'lastname', 'email', 'phone', 'language', "token","getWhatsappNotifiations"].includes(key))
