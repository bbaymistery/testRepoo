...
export const readyToCollectQuotationOptions = async (params = {}) => {
    ...
    (async () => {
        let { dispatch, setInternalState, router, journeyType, reservations, language, shouldNavigate = true, env, appData, message = "APL readyToCollectQuotationOptions()" } = params
        setInternalState({ ["quotation-loading"]: true })

        let log = await collectQuotationsAsync({ reservations, journeyType, env, message })
