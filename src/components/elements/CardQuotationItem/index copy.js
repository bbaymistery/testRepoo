...
const CardQuotationItem = (params = {}) => {
  ...
  const [selectedJourneyAccordionState, setSelectedJourneyAccordionState] = useState(null);

  const toggleJourneyAccordion = (e) => {
    //togglus yazsak biri otomatik acg galar
    if (e === active) return setSelectedJourneyAccordionState(null);
    setSelectedJourneyAccordionState(e);
  };
          ...
          if (Number(selectedQuotation?.carId) === Number(quotationImagesObj[item?.carId]?.id)) {
            ...
            >
              ...
              <div className={`${direction === 'rtl' ? styles.thirdcolumnDirection : ""} ${styles.column_third}`}>
                ...
                <button className={`btn btn_primary ${Number(selectedQuotation?.carId) === Number(carObject[item?.carId].id) ? styles.selectedBtn : ""}`}   >
