...
const QuotationResultsTaxiDeal = (props) => {
    ...
    let { direction, journeyType, language, quotations } = params
    const router = useRouter()

    const { appData } = useSelector(state => state.initialReducer)
    const carObject = Array.isArray(appData?.carsTypes) ? appData?.carsTypes?.reduce((obj, item) => ({ ...obj, [item.id]: item, }), {}) : {};
    const objectDetailss = Array.isArray(appData?.pointTypeCategories) ? appData.pointTypeCategories.reduce((obj, item) => ({ ...obj, [item.id]: JSON.parse(item.objectDetails), }), {}) : {};
    ...
    useEffect(() => {
        localStorage.setItem("path", `/${router.query.linkname}`);
    }, [])
