...
const NewPaymentDesign = (props) => {
    let { env, totalPrice } = props
    ...
    // useRef hook'unu import ettiğinden emin ol (zaten etmişsin)
    const stripeRef = useRef(null);

    // Bu useEffect'i component içine ekle
    useEffect(() => {
        // Eğer iframe linki geldiyse ve ref tanımlıysa oraya kaydır
        if (iframeStripe?.length > 0 && stripeRef.current) {
            // Hafif bir gecikme (100ms) DOM'un tam oturmasını garanti eder
            setTimeout(() => {
                stripeRef.current.scrollIntoView({
                    behavior: "smooth",
                    block: "center" // "start" yaparsan en üste, "center" yaparsan ortaya alır
                });
            }, 100);
        }
    }, [iframeStripe]);
    ...
    useEffect(() => {
        ...
        updatedReservationsRef.current = reservations;
    }, [reservations]);
    return (
        <div className={styles.payment_container}>
            <div>
                <PaymentHeader strPaymentMethod={appData?.words["strPaymentMethod"]} />
                <div className={styles.payment_options_grid}>
                    {/* cash */}
                    ...
                    >
                        <input type="radio" name="payment_method" value="cash" className={styles.payment_radio_input} checked={selectedMethod === 1} readOnly onFocus={(e) => e.target.blur()} />
                        <div className={styles.payment_card_box}>
                            <div className={`${styles.icon_wrapper} ${styles.dark_blue}`}>
                                <i className="fa-solid fa-money-bill-wave"></i>
                            </div>
                            <div className={styles.text}>
                                <p className={styles.payment_card_box_text_1}>
                                    {appData?.words["strToDriverCashTitle"]}
                                </p>
                                <p className={styles.payment_card_box_text_2}>
                                    {'Reserve now, pay later'}
                                </p>
                                <p className={styles.payment_card_box_text_3}>
                                    {appData?.words["strTotalPrice"] || 'Total Amount to Pay'} : {totalPrice}
                                </p>
                            </div>
                    ...
                    {/* card */}
                    <label className={styles.payment_radio_label} onClick={() => {
                        setSelectedMethod(7);
                        startPayment(7);
                    }} >
                        <input
                            ...
                            name="payment_method"
                            value="card"
                            className={styles.payment_radio_input}
                            checked={selectedMethod === 7}
                            readOnly
                        ...
                        <div className={styles.payment_card_box}>
                            ...
                            <div className={styles.text}>
                                <p className={styles.payment_card_box_text_1}>
                                    {appData?.words["strPaybycard"] || 'Credit Card'}
                                </p>

                                <p className={styles.payment_card_box_text_2}>
                                    {appData?.words["strSecurePaymentViaStripe"] || 'Secure Payment via Stripe'}
                                </p>
                                <p className={styles.payment_card_box_text_3}>
                                    {appData?.words["strTotalPrice"] || 'Total Amount to Pay'} : {totalPrice}
                                </p>
                    ...
                    >
                        ...
                        <div className={styles.payment_card_box}>
                            ...
                            <div className={styles.text}>
                                <p className={styles.payment_card_box_text_1}>
                                    {appData?.words["strPaywithPayPal"] || 'PayPal'}
                                </p>
                                <p className={styles.payment_card_box_text_2}>
                                    {appData?.words["strFastEasyAndSecure"] || 'Fast , Easy and secure'}
                                </p>
                                <p className={styles.payment_card_box_text_3}>
                                    {appData?.words["strTotalPrice"] || 'Total Amount to Pay'} : {totalPrice}
                                </p>
                    ...
                    {iframeStripe?.length > 0 && (
                        <div className={styles.stripe_container} ref={stripeRef}>
                            <iframe
                                src={iframeStripe}
...
export default NewPaymentDesign;
