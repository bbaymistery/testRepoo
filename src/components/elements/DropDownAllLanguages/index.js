...
const DropDownAllLanguages = ({ languageStatus, handleLanguage, expandedMenu }) => {
    const { appData } = useSelector(state => state.initialReducer)

    return (
        <div className={styles.all_languages} expand={String(expandedMenu)} style={{ opacity: `${languageStatus ? 1 : 0}`, visibility: `${languageStatus ? "visible" : "hidden"}` }} >
