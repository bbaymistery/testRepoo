...
const RadioButton = ({ setInternalState, internalState }) => {
  ...
  return (
    <span className={`${styles.radio_button_div} ${direction}`}>
      ...
      <label htmlFor="check1" onClick={() => handleInputRadioChange(0)} id="check1">{appData.words["seOneWayOption"]}</label>
      &nbsp;&nbsp;
      &nbsp;&nbsp;
      <input type="radio" id="check2" onChange={() => handleInputRadioChange(1)} checked={parseInt(journeyType) === 1 ? true : false} />
      <label htmlFor="check2" onClick={() => handleInputRadioChange(1)} >{appData.words["seReturnOption"]}</label>
    </span>
  );
};
