...
const SelectedPointsOnHomePage = dynamic(() => import('../../elements/SelectedPointsOnHomePage'), {
    ssr: false, // opsiyonel: eğer sadece client'ta kullanıyorsan
    loading: () => <></>, // opsiyonel: istersen loading gösterebilirsin
});
    ...
    return (
        <div className={styles.main_search_wrapper}>
            ...
            <div className={styles.search_menu}>
                {!points.length ? <p className={direction}>{locationText}</p> : null}
                {points.length ? <p className={`${styles.point_title} ${direction}`}>{pointTitle}</p> : null}
