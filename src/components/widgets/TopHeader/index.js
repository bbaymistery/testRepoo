        ...
        <div className={styles.header_flex_div}>
                  ...
                  {navigator.map((item, index) => {
                    ...
                    return (
                      ...
                      <li key={innerText} className={`${styles.li_item} ${type === "list" ? styles.has_children : ""}`}>
                        <a onClick={() => gotoHomePage(index)} tabIndex="-1" href={`${language === 'en' ? `${path}` : `${language}${path}`}`} title={title} className={`${!path.length ? styles.nocursor : ""}`}>
                          <span>{innerText}</span>
                  ...
                  {navigatorMobile.map((item, index) => {
                    ...
                    return (
                      <li key={innerText} className={`${styles.li_item} ${type === "list" ? styles.has_children : ""}`} id="navLink">
                        <a onClick={() => handleClickNavLinkMobileMenuNotList({ index })} tabIndex="-1" href={`${language === 'en' ? `${path}` : `${language}${path}`}`} title={title} className={`${!path.length ? styles.nocursor : ""}  ${firstChild ? styles.first_child_a : ""} `} >
                          <span>{innerText}</span>
                          {/* <span>{index === 0 ? appData?.words[innerText] : innerText}</span> */}
                          {type === "list" ? <i className="fa-solid fa-angle-down"></i> : ""}
                        </a>
                            ...
                            {list.map((item) => {
                              ...
                              return (
                                <li tabIndex="-1" key={listInnerText} className={`${styles.li_item} ${!listPath.length ? styles.nocursor : ""}  `}>
                                  <a onClick={() => handleClickNavLinkMobileMenuList({ hasTaxiDeals })} href={`${language === 'en' ? `${listPath}` : `${language}${listPath}`}`} title={listTitle} tabIndex="-1">
                                    <span>{listInnerText}</span>
                                  </a>
                                </li>
                              )
          ...
          <div className={styles.right_items}>
            ...
            <div className={`${router.asPath === "/drivers-wanted" ? "no_cursor" : ""} ${styles.language_dropdown}`}>
              <div className={styles.top} onClick={setOpenLanguageDropdown}>
                <div className={styles.img_div}>
                  <Image src={`/languages/${langFlag}.gif`} width={20} height={11} priority alt={langFlag} />
                </div>
                <span className={styles.lang_text}>{appData?.languages[langIndex]?.innerText}<i className="fa-solid fa-angle-down"></i></span>
                <OutsideClickAlert onOutsideClick={outsideClickDropDown}>
                  <div ref={languagesDivRef} className={styles.all_languages}>
                    {appData?.languages.map((item, index) => {
