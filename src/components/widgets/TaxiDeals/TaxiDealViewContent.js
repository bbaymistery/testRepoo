...
import { taxidealsImages } from '../../../constants/taxideals'
                        ...
                        return (
                            <a data-id="a" title={item?.pageTitle} href={item.pathname} className={`${styles.card}`} key={item.id}>
                                <div className={styles.card_image_div}>
                                    <Image src={`${taxidealsImages?.[index]?.image}`} className={styles.img} fill priority alt={item.pageTitle} sizes="(max-width: 768px) 100vw, 50vw" />
                                </div>
                    ...
                    {points.length > 1 ?
                        filteredDatas.slice(4, 8).map((item, index) => {
                            ...
                            return (
                                <a data-id="a" title={item?.pageTitle} href={item.pathname} className={`${styles.card}`} key={item.id}>
                                    <div className={styles.card_image_div}>
                                        <Image src={`${taxidealsImages?.[index + 4]?.image}`} className={styles.img} fill priority alt={item.pageTitle} sizes="(max-width: 768px) 100vw, 50vw" />
