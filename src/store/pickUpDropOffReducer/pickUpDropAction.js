...
export const collectPickUpPoints = (inpValue, index, journeyType) => async (dispatch, getState) => {
...
export const addItemToSelectedList = (requestBody, pickOrDrop, journeyType, objectDetails) => async (dispatch) => {
  const env = await fetchConfig();
...
export const gettingQuotations = (router, journeyType, updateInsideQuotation) => async (dispatch, getState) => {
  const env = await fetchConfig();
...
export const getAppData = (props) => async (dispatch) => {
...
// * EDITINGS ITEMSSSS
export const getAllDatasForEditing = (datas) => async (dispatch) => {
  const env = await fetchConfig()
  //transfer
