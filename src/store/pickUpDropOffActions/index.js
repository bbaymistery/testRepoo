...
import COPY_PICK_UP_DETAILS_FOR_TOURS_DROPOFF from "./COPY_PICK_UP_DETAILS_FOR_TOURS_DROPOFF";
import SET_QUOTATION_ON_SPECIAL_CASE from "./SET_QUOTATION_ON_SPECIAL_CASE";
import SET_WHATSAP_PASSENGER_NOTIFICATION from "./SET_WHATSAP_PASSENGER_NOTIFICATION.JS";
...
let initialReservationState = [{
  ...
  passengerDetails: {
    ...
    email: "",
    phone: "",
    getWhatsappNotifiations: 1, //2 > off &&  1 > on
  },
  paymentDetails: {
    token: "",
    paymentType: "",
...
export const pickUpDropOffActions = (state = INITIAL_STATE, action) => {
  switch (action.type) {
    ...
    case "SET_WHATSAP_PASSENGER_NOTIFICATION": {
      return SET_WHATSAP_PASSENGER_NOTIFICATION({ state, action })
    }
