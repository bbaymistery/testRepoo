...
const hours = ["Select-hour", ...Array.from({ length: 24 }, (_, index) => {
...
const ParkingCalculator = (props) => {
    ...
    const handleDriverPickUpChange = (event) => {
        setDriverPickUp(event.target.value);
    }

    const handleHowManyMinutes = (event) => {
        const input = event.target.value;
        const regex = /^[0-9]*$/; // Regular expression to match only digits
        if (input.match(regex)) setHow_many_minutes(input);
    };
    // Function to add minutes to a given time string (hh:mm)
    useEffect(() => {
                        ...
                        <div className={styles.content}>
                            ...
                            <div className={styles.input_informations_div}>
                                <div className={styles.landing_time_div}>
                                    <DropDown heading="Landing time" options={hours} selectedOption={landingHourSelection} setSelectedOption={setLandingHourSelection} />
                                ...
                                <div className={styles.driver_pickup_div}>
                                    <DropDown disable={how_many_minutes && freetimeEnds ? false : true} heading="Driver actual pick up" options={hours} selectedOption={driverActualPickupHourSelection} setSelectedOption={setdriverActualPickupHourSelection} />
