...
import usePageLeave from '../../hooks/usePageLeave.js'
import { urlWithLangAtribute } from '../../helpers/urlWithLangAtrribute.js'
import Head from 'next/head.js'
import Loading from '../../components/elements/Loading/index.js'
import { getCookie } from '../../helpers/cokieesFunc.js'
import usePageGuard from '../../hooks/usePageGuard.js'
...
const QuotationResults = (props) => {
  ...
  let { sessionToken: reducerSessionToken, journeyType, direction, language, quotations, tokenForArchieve } = params
  const reservationsRef = useRef(reservations);
  const { appData } = useSelector(state => state.initialReducer)
  const [updatedQuotation, setUpdatedQuotation] = useState(false)
  const { nexturls, previousUrls, currentUrls } = urlWithLangAtribute({ languages: appData.languages, previousUrl: "/", nextUrl: "/transfer-details", currentUrl: router.asPath })
  const isDataValid = reservations && reservations.length > 0 && reservations[0]?.selectedPickupPoints && reservations[0]?.selectedPickupPoints.length > 0;
  const { isPageAuthorized } = usePageGuard(isDataValid, () => {
    // Burası sadece sayfa yetkilendirildiğinde (isValid = true ise) çalışır
    if (didMountRef.current) {
      if (reservations?.[0]?.transferDetails?.transferDateTimeString) {
        getQuotations();
      }
    }
  });
  ...
  const fetchArchieveToken = async (params = {}) => {
    ...
    // console.log({current: reservationsRef.current});
    console.log({ current: reservations });

    const url = `${env.apiDomain}/api/v1/sessions/add`;
    const headers = { "Content-Type": "application/json" }
    const body = JSON.stringify({ token: tokenForArchieve, details: reservations, stage })
  ...
  const onChangeSetDateTimeHandler = (params = {}) => {
    ...
    setUpdatedQuotation(true)
    setTimeout(() => {
      setUpdatedQuotation(false)
    }, 1);
  }
  ...
  const getQuotations = (params = {}) => {
    ...
    if (errorHolder.status === 200) readyToCollectQuotationOptions({
      ...
      env,
      appData,
      message: "APL QuotationResults - getQuotations()"
    })
    if (updatedQuotation) {
      fetchArchieveToken({ stage: "UPDATE_QUOTATION_DETAILS_PAGE" })
  ...
  useEffect(() => {
    ...
    fetchArchieveToken({ stage: "LANDED_INTO_QUOTATION_DETAILS_PAGE" })
  ...
  usePageLeave({
    ...
    isEnabled: false, // İstersen bir koşula bağlayabilirsin
                        ...
                        {(() => {
                          ...
                          return showMap ? (
                            <div className={styles.map_direction}>
                              <Map env={env} datas={quotations[index]} appData={appData} selectedPickPoints={selectedPickupPoints} selectedDroppOfPoints={selectedDropoffPoints} />
                            </div>
                          ) : null;
                        })()}
                      </div>
